...
When created Cards, they are shown on the main top container. Now when there is a Card, there can also be added a transaction per specific Card.
...
Every Card can have transactions. While adding transactions it is possible to pick one or many categories for that specific transaction. The categories have a prefilled state but it is also possible to add new or delete a category.
...
<img src="images/iphone13.png" widht= 150 height = 300  hspace="0">
...
# iPad - Main - Add Card
The app also suppoert portrait/landscape mode for iPad. The Main screen shows a specific UI for iPad. The user has the same functionalities as on the iPhone therefore on the start he needs to add a Card.
